...
export default function Navbar() {
    ...
    return (
        <header className="bg-gray-100 relative p-5 my-2 flex justify-around items-center">
            <span className="logo text-2xl">PicArts</span>
            <form className="" onSubmit={searchSubmitHandler}>
                ...
                <div className="suggestions  max-h-[200px] w-[300px] overflow-auto  bg-slate-300 ">
                    {filterSuggestion.map((suggestion, ind) =>
                        suggestions !== input ? (
                            <p
                                key={ind}
                                className="suggestion-item absolute bg-gray-300 w-[244px] px-2 py-1 hover:bg-slate-500"
